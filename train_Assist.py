...
def get_corr_data(pro_num):
    ...
    list1 = []
    tmp = []
    pro_pro_ = open('./autodl-tmp/diff_rel_m.txt',"r")
    for i in pro_pro_:
        j = i.strip().split(",")
...
def get_q_matrix(embed_size):
    with open("./autodl-tmp/q_matrix.txt","r",encoding='utf-8') as f:
        q_matrix = []
...
if __name__ == "__main__":
    ...
    data = pd.read_csv("./autodl-tmp/train_Eedi.csv")
    ...
    test_set  = pd.read_csv("./autodl-tmp/test_Eedi.csv")
