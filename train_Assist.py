...
def train2(train_data, val_data, pro_num, timestamp, timespan,  model, optimizer, logger, saver, skill_nums, q_matrix, lamda, beta, num_epochs, batch_size, grad_clip):
    
    step = 0
    metrics = Metrics()
    corr_data = get_corr_data2(pro_num)
    result_train = []
    ...
    for epoch in range(num_epochs):
        ...
        for item_inputs, label_inputs, item_ids, timestamp, labels in train_batches: 
            ...
            preds, weights = model(skill_nums, q_matrix, item_inputs, label_inputs, item_ids, torch.Tensor(rel), time)
        ...
        for item_inputs, label_inputs, item_ids, timestamp, labels in val_batches:
            ...
            item_ids = item_ids.cuda()
            with torch.no_grad():
                preds,weights = model(skill_nums, q_matrix,item_inputs, label_inputs, item_ids, torch.Tensor(rel).cuda(), time.cuda())
...
if __name__ == "__main__":
    ...
    while True:
        ...
        train2(train_data, test_data1, pro_num, timestamp, args.timespan, model, optimizer, logger, saver, skill_num, q_matrix, args.lamda,args.beta, args.num_epochs,
    ...
    test_batches = prepare_batches_test(test_data1, args.test_size, randomize=True)
    
    corr_data = get_corr_data2(num_items)
    ...
    for item_inputs, label_inputs, item_ids, timestamp, labels in test_batches:
        ...
        with torch.no_grad():
                preds,weights = model(skill_num, q_matrix,item_inputs, label_inputs, item_ids, torch.Tensor(rel).cuda(), time.cuda())
                preds = torch.sigmoid(preds).cpu()
